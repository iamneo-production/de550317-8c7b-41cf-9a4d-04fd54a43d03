...
MAX(CAST(REGEXP_REPLACE(WITHDRAWAL_AMT, '[^0-9.]','') AS NUMBER))
     AS HIGHEST_AMOUNT_DEBITED
    FROM WITHDRAW_Q WHERE WITHDRAWAL_AMT IS NOT NULL GROUP BY EXTRACT(YEAR FROM "DATE");
...
MIN(
    CAST(REGEXP_REPLACE(WITHDRAWAL_AMT ,'[^0-9.]','') AS NUMBER))
     AS LOWEST_AMOUNT_DEBITED FROM WITHDRAW_Q
    WHERE WITHDRAWAL_AMT IS NOT NULL GROUP BY EXTRACT(YEAR FROM "DATE");
...
SELECT DISTINCT EXTRACT (YEAR FROM "DATE") AS YEAR,CAST(
    REGEXP_REPLACE(WITHDRAWAL_AMT,'[^0-9.]','') AS NUMBER) 
AS FIRST_FIVE_LARGEST_WITHDRAWAL_TRANSACTIONS FROM WITHDRAW_Q
WHERE EXTRACT(YEAR FROM "DATE")=2018 AND 
CAST(REGEXP_REPLACE(WITHDRAWAL_AMT,'[^0-9.]','') AS NUMBER) 
IS NOT NULL ORDER BY CAST(
    REGEXP_REPLACE(WITHDRAWAL_AMT,'[^0-9.]','')
    AS NUMBER)
