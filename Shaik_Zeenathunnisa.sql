...
--QUERY-1  --HIGHEST AMOUNT DEBITED FROM THE BANK IN EACH YEAR

SELECT YEAR,
    MAX(BANK_AMOUNT) AS HIGHEST_AMOUNT_DEBITED
    FROM BANK_DATABASE_VIEW 
GROUP BY YEAR;

 --QUERY-2 --LOWEST AMOUNT DEBITED FROM THE BANK IN EACH YEAR 

SELECT YEAR,
    MAX(BANK_AMOUNT) AS LOWEST_AMOUNT_DEBITED 
    FROM BANK_DATABASE_VIEW
GROUP BY YEAR;

--QUERY-3 --FIFTH HIGHEST AMOUNT DEBITED IN EACH YEAR
...
(
    ...
    BANK_AMOUNT DESC) AS R FROM BANK_DATABASE_VIEW
)
WHERE R=5;

--QUERY -4 --TO COUNT THE WITHDRAWAL TRANSACTIONS BETWEEN MAY 5,2018 AND MARCH 7,2019
...
SELECT DISTINCT YEAR,BANK_AMOUNT AS 
FIRST_FIVE_LARGEST_WITHDRAWAL_TRANSACTIONS FROM BANK_DATABASE_VIEW
